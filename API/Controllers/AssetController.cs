...
namespace API.Controllers
{
    ...
    public class AssetController : BaseApiController
    {
        ...
        //         // Create a dictionary to map enum values to display names
        //         var statusMap = new Dictionary<AssetStatus, string>
        // {
        //     { AssetStatus.Normal, "Normal" },
        //     { AssetStatus.GoodCondition, "Good Condition" },
        //     { AssetStatus.Broken, "Broken" },
        //     { AssetStatus.UnderRepair, "Under Repair" }
        // };

        //         // Get the display name for the updated status
        //         var updatedStatus = statusMap.TryGetValue(updatedAsset.AssetStatus, out var displayName)
        //             ? displayName
        //             : updatedAsset.AssetStatus.ToString();

        //         // Modify the asset's status to use the display name
        //         updatedAsset.AssetStatus = (AssetStatus)Enum.Parse(typeof(AssetStatus), updatedStatus);

        //         return Ok(updatedAsset); // Return the updated asset with the display name for the status
        //     }
        //     else
        //     {
        //         return StatusCode(StatusCodes.Status500InternalServerError, "Failed to update asset status"); // Return 500 Internal Server Error if the update fails
